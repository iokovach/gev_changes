...
void generateIC_prevolution(metadata & sim, icsettings & ic, cosmology & cosmo, const double fourpiG, double & a, double & tau, double & dtau, double & dtau_old, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_phi, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij, parameter * params, int & numparam)
{
	...
	{   
	    if (cycle > 0 && (1. / a < ic.z_relax + 1. || 1. / a <= sim.z_in + 1.))
	    {
	        ...
	        prepareFTsource<Real>(*phi, *chi, *source, cosmo.Omega_cdm + cosmo.Omega_b, *source, 3. * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) * dx * dx / dtau_old, fourpiG * dx * dx / a, 3. * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) * dx * dx);
	        plan_source->execute(FFT_FORWARD);
	        solveModifiedPoissonFT(*scalarFT, *scalarFT, 1. / (dx * dx), 3. * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) / dtau_old);
		...
		if (relax_cycles > 0 && ic.Cf > sim.Cf)
	        dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx + (a - (1. / (ic.z_relax + 1.))) * ((sim.Cf * dx < sim.steplimit / Hconf(a, dm, rad, dcdm, fourpiG, cosmo)) ? (sim.Cf * dx) : (sim.steplimit / Hconf(a, dm, rad, dcdm, fourpiG, cosmo)))) / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
		else if (relax_cycles > 0 && ic.Cf * dx > sim.steplimit / Hconf(a, dm, rad, dcdm, fourpiG, cosmo))
	        dtau = (((1. / (sim.z_in + 1.)) - a) * ic.Cf * dx + (a - (1. / (ic.z_relax + 1.))) * sim.steplimit / Hconf(a, dm, rad, dcdm, fourpiG, cosmo)) / ((1. / (sim.z_in + 1.)) - (1. / (ic.z_relax + 1.)));
		else
			dtau = ic.Cf * dx;
	...
	if (sim.gr_flag == 0)
	{
		...
		rescale = 3. * Hconf(a, dm, rad, dcdm, fourpiG, cosmo)  * M_PI;
		...
		rescale = 3. * Hconf(0.99 * a, dm, rad, dcdm, fourpiG, cosmo) * M_PI;
		mean_q = -99.5 * Hconf(0.995 * a, dm, rad, dcdm, fourpiG, cosmo);
