...
void generateIC_basic(metadata & sim, icsettings & ic, cosmology & cosmo, const double fourpiG, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_phi, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij, parameter * params, int & numparam)
{
	...
	else					// initial displacements and velocities are set by individual transfer functions
	{
		...
		for (i = 0; i < tk_d1->size; i++) // construct phi
			temp1[i] = (1.5 * (Hconf(a, dm, rad, dcdm, fourpiG, cosmo) * Hconf(a, dm, rad, dcdm, fourpiG, cosmo) - Hconf(1., dm, rad, dcdm, fourpiG, cosmo) * Hconf(1., dm, rad, dcdm, fourpiG, cosmo) * a * a * cosmo.Omega_Lambda) * tk_d1->y[i] + rescale * tk_t1->y[i] / tk_d1->x[i] / tk_d1->x[i]) * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i];
		...
		if (ic.tkfile[0] == '\0')
		{
			if (sim.gr_flag == 0)
			{
				...
				for (i = 0; i < tk_d1->size; i++) // construct gauge correction for N-body gauge velocities
					temp1[i] = -99.5 * Hconf(0.995 * a, dm, rad, dcdm, fourpiG, cosmo) * (3. * temp1[i] * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] + (temp2[i] + 3. * Hconf(0.99 * a, dm, rad, dcdm, fourpiG, cosmo)  * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i]));
	...
	{	
		rescale = a / Hconf(a, dm, rad, dcdm, fourpiG, cosmo) / (1.5 * Omega_m(a, dm, rad, dcdm, cosmo) + 2. * Omega_rad(a, dm, rad, dcdm, cosmo));
	...
	for (p = 0; p < cosmo.num_ncdm; p++)
	{
		...
		{		
			rescale = a / Hconf(a, dm, rad, dcdm, fourpiG, cosmo) / (1.5 * Omega_m(a, dm, rad, dcdm, cosmo) + Omega_rad(a, dm, rad, dcdm, cosmo));
